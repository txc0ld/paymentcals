...
export function ExplainabilityTabs({
  ...
  return (
    <section aria-label="How this was calculated" className="nexus-panel min-w-0 overflow-hidden">
      <div
        role="tablist"
          ...
          result.trace && result.trace.steps.length > 0 ? (
            <ol className="flex flex-col gap-4">
              {result.trace.steps.map((step) => (
                <li key={step.id} className="nexus-panel-soft grid min-w-0 gap-1 p-4">
              ...
              {result.sources.map((source) => (
                <li key={source.sourceId} className="grid gap-1 py-3 first:pt-0 last:pb-0">
                  ...
                  <a
                    ...
                    target="_blank"
                    className="inline-flex min-h-11 items-center break-all font-mono text-[12px] text-info underline decoration-hairline-strong underline-offset-2 hover:decoration-current focus-visible:rounded-[var(--pc-radius-control)] focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-focus"
                  >
                    {source.url}
                  </a>
        ...
        {active === "Limitations" ? (
          <ul className="flex list-none flex-col gap-2">
            {limitations.map((limitation) => (
              <li key={limitation} className="nexus-panel-soft p-4 text-[13px] leading-5 text-ink-2">
                {limitation}
              </li>
            ))}
          </ul>
